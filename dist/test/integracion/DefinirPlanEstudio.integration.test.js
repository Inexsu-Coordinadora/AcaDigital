...
let programaRepo;
let asignaturaRepo;
let definirPlanEstudioUseCase;
const PROGRAMA_ID = 'PROG-1';
const ASIGNATURA_ID = 42;
const programaValido = new ProgramaAcademico('Ingeniería de Sistemas', 'Programa de 10 semestres', NivelEducativo.DOCTORADO, Modalidad.PRESENCIAL, new Duracion(10, 'semestres'), PROGRAMA_ID);
const asignaturaValida = new Asignatura('Estructura de Datos', 4, TipoAsignatura.TEORICA, ASIGNATURA_ID);
const dtoValido = {
...
describe('INTEGRACION: DefinirPlanEstudioUseCase', () => {
    beforeEach(async () => {
        ...
        definirPlanEstudioUseCase = new DefinirPlanEstudioUseCase(planRepo, programaRepo, asignaturaRepo);
    ...
    it('deberia lanzar ErrorConflicto si el vinculo ya existe', async () => {
        ...
        await expect(definirPlanEstudioUseCase.ejecutar(dtoValido)).rejects.toThrow(`La asignatura ya esta registrada en este programa`);
    });
    ...
    it('deberia lanzar ErrorNoEncontrado si el programa academico no existe', async () => {
        ...
        await expect(definirPlanEstudioUseCase.ejecutar(dtoProgramaInvalido)).rejects.toThrow('Programa academico no encontrado');
    });
    ...
    it('deberia lanzar ErrorNoEncontrado si la asignatura no existe', async () => {
        ...
        await expect(definirPlanEstudioUseCase.ejecutar(dtoAsignaturaInvalida)).rejects.toThrow('Asignatura no encontrada');
    });
    ...
    it('deberia lanzar ErrorValidacion si el semestre/nivel es invalido (0 o negativo)', async () => {
        ...
        await expect(definirPlanEstudioUseCase.ejecutar(dtoSemestreInvalido)).rejects.toThrow('El semestre/nivel debe ser un entero positivo');
    });
    ...
    it('deberia lanzar ErrorValidacion si la carga de creditos es invalido (0 o negativo)', async () => {
        ...
        await expect(definirPlanEstudioUseCase.ejecutar(dtoCreditosInvalidos)).rejects.toThrow('Los creditos/carga horaria deben ser mayor que cero');
    });
});
//# sourceMappingURL=DefinirPlanEstudio.integration.test.js.map