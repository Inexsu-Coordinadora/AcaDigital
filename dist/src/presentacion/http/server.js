...
//periodo academico
import { CrearPeriodoUseCase, ObtenerPeriodosUseCase, ObtenerPeriodoPorIdUseCase, ActualizarPeriodoUseCase, EliminarPeriodoUseCase } from '../../core/aplicaciones/periodo-academico/index.js';
...
//programa academico
import { CrearProgramaAcademicoUseCase, ListarProgramasAcademicosUseCase, ObtenerProgramaAcademicoPorIdUseCase, ActualizarProgramaAcademicoUseCase, EliminarProgramaAcademicoUseCase } from '../../core/aplicaciones/programa-academico/index.js';
...
import { DefinirPlanEstudioUseCase } from '../../core/aplicaciones/plan-estudio/index.js';
...
const definirPlanEstudioUseCase = new DefinirPlanEstudioUseCase(planEstudioRepository, programaRepository, asignaturaRepository);
// Oferta Académica
const ofertaRepositorio = new OfertaAcademicaPGRepositorio();
const ofertarAsignaturaUseCase = new OfertarAsignaturaUseCase(ofertaRepositorio, periodoRepository, programaRepository, asignaturaRepository);
// --- Servidor Fastify ---
...
server.setErrorHandler((error, request, reply) => {
    ...
    if (error.code === 'FST_ERR_VALIDATION' && error.validation) {
        const validationError = error.validation.find((e) => e);
        let detailMessage;
        if (validationError) {
...
server.register(async (instance, options) => {
    registerPeriodoAcademicoRoutes(instance, crearPeriodoUseCase, listarPeriodosUseCase, obtenerPeriodoPorIdUseCase, actualizarPeriodoUseCase, eliminarPeriodoUseCase);
